...
class CausalConv1d(nn.Conv1d):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x_pad = F.pad(
            x,
            (self.__padding * 2 - self.__output_padding, 0),
        )
...
class CausalTransposeConv1d(nn.ConvTranspose1d):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return super().forward(x)[
            ..., : -(self.__padding * 2 - self.__output_padding)
        ]
...
class CausalEncoder(nn.Module):
    def __init__(
        ...
        layers: list[nn.Module] = [WNCausalConv1d(1, d_model, kernel_size=7, padding=3)]
...
class NoiseBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        noise = torch.randn((batch, 1, time), device=x.device, dtype=x.dtype)
        h = self.linear(x)
        return x + noise * h
...
class SampleRateConditionLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, sr_cond: torch.Tensor) -> torch.Tensor:
        if self.cond_type in {"scale_bias", "scale_bias_init"}:
            x = (
                x * self.scale_embed(sr_cond).unsqueeze(-1)
                + self.bias_embed(sr_cond).unsqueeze(-1)
            )
...
class CausalDecoder(nn.Module):
    def __init__(
        ...
        if sr_bin_boundaries is None:
            ...
            self.register_buffer(
                "sr_bin_boundaries",
                torch.tensor(sr_bin_boundaries, dtype=torch.int32),
            )
            self.sr_bin_buckets = len(sr_bin_boundaries) + 1
            cond_layers: list[nn.Module | None] = []
    ...
    def get_sr_idx(self, sr: torch.Tensor) -> torch.Tensor:
...
class AudioVAEConfig(BaseModel):
    ...
    use_noise_block: bool = False
    sr_bin_boundaries: list[int] | None = [20000, 30000, 40000]
    cond_type: str = "scale_bias"
    cond_dim: int = 128
    cond_out_layer: bool = False
...
class AudioVAE(nn.Module):
    def __init__(self, config: AudioVAEConfig | None = None):
        ...
        latent_dim = (
            ...
            else config.encoder_dim * (2 ** len(config.encoder_rates))
        )
    ...
    def decode(
        self, z: torch.Tensor, sr_cond: torch.Tensor | None = None
    ) -> torch.Tensor:
...
class StreamingVAEDecoder:
    ...
    def _patch_causal_conv(self, mod: nn.Module, pad_size: int):
        states = self._states
        key = id(mod)
        orig = mod.forward
    ...
    def _patch_transpose_conv(self, mod: nn.Module, ctx: int, trim: int):
        states = self._states
        key = id(mod)
        orig = mod.forward
...
AudioVAEConfigV2 = AudioVAEConfig
AudioVAEV2 = AudioVAE
